...
void runMasterFile(const std::string &idirectory, const std::string &ipathId) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    case WD: {
      ...
      const double xTarg = ((masterFunctionDoubleXY *)masterFunctionList[i])->x;
      const double yTarg = ((masterFunctionDoubleXY *)masterFunctionList[i])->y;
      double xDiff = (xTarg - currentPos.x.convert(okapi::inch));
      double yDiff = (yTarg - currentPos.y.convert(okapi::inch));
